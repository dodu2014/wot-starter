...
const { loading, send: sendGetListRequest } = useRequest(
  (num?: string, page: number = 1) =>
    Webapi_Base.article.getArticleList({ params: { categoryNum: num, page, pageSize: 20 } }),
  { immediate: false },
).onError(error => {
  toast.error(error.error?.message)
})
...
onReachBottom(async () => {
  if (page.value < totalPage.value) await loadList(num.value, page.value + 1)
})
</script>
