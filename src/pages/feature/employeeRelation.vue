...
const { send: sendBindRequest } = useRequest(
  (status: string, nickName?: string) =>
    Webapi_Weixin.wxOpen.onCheckEmployeeRelation({
      params: {
...
const { bindStatus, bindEmployeeRelation, checkEmployeeRelation, requestSubscribeEmployeeMessage } =
  useEmployeeMessage({
...
const { send: sendSubscribeEmployeeMessage } = useRequest(
  (openId: string) =>
    Webapi_Demo.subscribeMessage.sendSubscribeEmployeeMessageTo({ params: { openId } }),
  { immediate: false },
)
...
onLoad(async () => {
  ...
  if (logined && wxUserInfo.value?.openId) handleCheck()
})
</script>
...
<template>
  <view class="flex-col flex-1 justify-center gap-y-3 p-4">
    ...
    <view class="text-center"> 用工关系绑定状态：{{ bindStatus || '未知' }} </view>
    <wd-divider />
    <wd-button block @click="handleBind"> 请求用工关系绑定 </wd-button>
    <wd-button block @click="handleCheck"> 检测用工关系状态 </wd-button>
    <wd-button block @click="handleRequestSubscribe"> 单独订阅指定消息 </wd-button>
    <wd-button block @click="testSendMessage"> 测试 `工单状态变更提醒` 模板消息 </wd-button>
  </view>
</template>
