...
<template>
  <view class="min-h-screen flex-col gap-y-3 py-3">
    <!-- 头部介绍 -->
    <wd-card class="!mb-0" custom-content-class="py-4 text-center">
...
console.log(counterStore.count) // 访问状态` }}
    ...
    <demo-block title="持久化演示 - 用户信息">
      <template #description>
        <wd-notice-bar
          ...
          :scrollable="false"
        >
          <view class="mb-1 text-3.5 font-bold">
            ✅ 持久化存储
          </view>
    ...
    <!-- 持久化原理 -->
    <demo-block title="持久化实现原理">
      <view class="text-4 text-gray-800 font-bold dark:text-[var(--wot-dark-color)]">
        持久化插件
      </view>
      <view class="text-3">
        项目使用自定义的 Pinia 插件实现持久化，位于
        <text class="text-blue-600 font-mono dark:text-blue-400">
