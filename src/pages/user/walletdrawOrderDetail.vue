...
definePage({
  name: 'user-wallet-withdraw-order-detail',
  layout: 'default',
  style: {
    navigationBarTitleText: '提现单详情',
    enablePullDownRefresh: true,
  },
  needLogin: true,
})

usePageTitle('pages.user.withdrawOrder.detailTitle')
...
<template>
  <view class="flex-col">
    ...
    <view v-if="loading === false && !modelList.length" class="h-65vh flex-center flex-col">
      <wd-empty icon="no-content" :tip="$t('pages.user.withdrawOrder.noContent')" />
    </view>
