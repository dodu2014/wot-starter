...
/* eslint-disable */
/**
 * TNT.WebApi 接口文档 | App 模块 - version v10.0.5.0
 *
...
export const createApis = (alovaInstance: Alova<AlovaGenerics>, configMap: any) => {
  const Apis = new Proxy({} as Webapi_App, {
    get(_, property) {
...
export const mountApis = (Apis: Webapi_App) => {
  // define global variable `Apis`
  (globalThis as any).Webapi_App = Apis;
};
...
type MethodConfig<T> = AlovaMethodCreateConfig<
  ...
  T
>;
type APISofParameters<Tag extends string, Url extends string> = Tag extends keyof Webapi_App
  ? Url extends keyof Webapi_App[Tag]
    ? Webapi_App[Tag][Url] extends (...args: any) => any
      ? Parameters<Webapi_App[Tag][Url]>
      : any
    : any
  : any;
