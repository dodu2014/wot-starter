...
export const createApis = (alovaInstance: Alova<AlovaGenerics>, configMap: any) => {
  const Apis = new Proxy({} as Webapi_Demo, {
    get(_, property) {
...
export const mountApis = (Apis: Webapi_Demo) => {
  // define global variable `Apis`
  (globalThis as any).Webapi_Demo = Apis;
};
...
type MethodConfig<T> = AlovaMethodCreateConfig<
  ...
  T
>;
type APISofParameters<Tag extends string, Url extends string> = Tag extends keyof Webapi_Demo
  ? Url extends keyof Webapi_Demo[Tag]
    ? Webapi_Demo[Tag][Url] extends (...args: any) => any
      ? Parameters<Webapi_Demo[Tag][Url]>
      : any
    : any
  : any;
