  ...
  <view class="min-h-screen py-3">
    ...
    <view class="mx-3 mb-3">
      <view class="rounded-3 px-5 py-6 text-center wot-bg-filled-oppo">
        ...
        <view class="mb-2 text-5 font-bold wot-text-text-main"> params 参数接收演示 </view>
        <view class="text-3.5 wot-text-text-secondary"> 演示如何接收和处理路由参数 </view>
      </view>
    </view>
    ...
    <demo-block title="接收到的参数" transparent>
      <view class="space-y-3">
        <view class="rounded-2 p-4 wot-bg-filled-oppo">
          <view class="mb-3 text-4 font-bold wot-text-text-main"> 原始参数 (option) </view>
    ...
    <demo-block title="API 说明" transparent>
      <view class="rounded-2 p-4 wot-bg-filled-oppo">
        <view class="mb-3 text-4 font-bold wot-text-text-main"> Params vs Query 的区别 </view>
        <view
        ...
        >
          ...
          <view class="text-3 text-orange-600 leading-relaxed dark:text-orange-200">
            在 @wot-ui/router 中，params 和 query 参数在实际效果上并无区别，都会以查询字符串形式放在
            URL 中。这种 API 设计主要是为了与 vue-router 保持一致。
          </view>
        </view>
        <view class="wot-bg-bg border rounded-2 p-3 wot-border-border-main">
