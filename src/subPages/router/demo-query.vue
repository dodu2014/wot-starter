  ...
  <view class="min-h-screen py-3">
    ...
    <view class="mx-3 mb-3">
      <view class="rounded-3 px-5 py-6 text-center wot-bg-filled-oppo">
        ...
        <view class="mb-2 text-5 font-bold wot-text-text-main"> query 参数接收演示 </view>
        <view class="text-3.5 wot-text-text-secondary"> 演示如何接收和处理查询参数 </view>
      </view>
    </view>
    ...
    <demo-block title="当前路由信息" transparent>
      <view class="rounded-3 p-4 wot-bg-filled-oppo">
        <view class="space-y-2">
          ...
          <view class="flex items-center justify-between border-b py-2 wot-border-border-main">
            <text class="text-3.5 wot-text-text-secondary"> 查询字符串: </text>
            <text class="text-3.5 font-mono wot-text-text-main">
              {{ JSON.stringify(route.query) }}
            </text>
          </view>
          <view class="flex items-center justify-between py-2">
            <text class="text-3.5 wot-text-text-secondary"> 接收方式: </text>
            <text class="text-3.5 wot-text-text-main"> onLoad(option) </text>
          </view>
        </view>
      </view>
    </demo-block>
    ...
    <demo-block title="接收到的查询参数" transparent>
      <view class="space-y-3">
        <view class="rounded-2 p-4 wot-bg-filled-oppo">
          <view class="mb-3 text-4 font-bold wot-text-text-main"> 原始查询参数 (option) </view>
          ...
          <view class="mt-3 text-3.5 wot-text-text-secondary">
            URL: /demo-query?keyword={{ (receivedQuery as any).keyword }}&type={{
              (receivedQuery as any).type
            }}
          </view>
        </view>

        <view v-if="decodedUser" class="rounded-2 p-4 wot-bg-filled-oppo">
          <view class="mb-3 text-4 font-bold wot-text-text-main"> 解码后的对象参数 </view>
          <view class="wot-bg-bg border rounded-2 p-3 wot-border-border-main">
            <text class="text-3 font-mono wot-text-text-secondary">
    ...
    <demo-block title="Query vs Params 说明" transparent>
      <view class="rounded-2 p-4 wot-bg-filled-oppo">
        <view class="mb-3 text-4 font-bold wot-text-text-main"> 两种参数传递方式说明 </view>
        <view
          class="mb-3 border border-orange-200 rounded-2 bg-orange-50 p-3 dark:bg-orange-900/20"
        >
          ...
          <view class="text-3 text-orange-600 leading-relaxed dark:text-orange-200">
            在 @wot-ui/router 中，query 和 params 参数都会放在 URL
